    ...
    for (const i in blockUnpacked.txList) {
      ...
      txUnpacked.txIns.each(({txHash, outN}) => {
        if (this.setTxOutSpent(txHash, outN, blockId)) {
          this.logBy('WLT', '{green-fg}IN added to WCA{/green-fg}', {txHash: Conv.bufToHex(txHash), outN, blockId})
        } else {
          this.logBy('WLT', '{red-fg}IN NOT added to WCA{/red-fg}', {txHash: Conv.bufToHex(txHash), outN, blockId})
        }
      })
      txUnpacked.txOuts.each(({address, value}, outN) => {
        if (this.addTxOut(address, blockId, txHash, outN, value, -1)) {
          this.logBy('WLT', '{green-fg}OUT added to WCA{/green-fg}', {txHash: Conv.bufToHex(txHash), outN, value, blockId})
        } else {
          this.logBy('WLT', '{red-fg}OUT NOT added to WCA{/red-fg}', {txHash: Conv.bufToHex(txHash), outN, value, blockId})
        }
      })
    }
    callback()
