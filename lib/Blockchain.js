...
const NAME_ADDRESSES = 'addresses.cache'
...
const PATH_ADDRESSES = BASE_PATH + NAME_ADDRESSES
...
class Blockchain extends Component {
  ...
  constructor() {
    ...
    this.addressOuts = BufferArray({
      step: 65536,
      fields: {
        address: {type: 'buffer', size: 25},
        blockId: {type: 'number', size: 4},
        txId: {type: 'number', size: 4},
    ...
    this.addressOutsTail = SteppedBuffer(16384)
    ...
    this.writeAddressesCache = () => {
      return new Promise((resolve, reject) => {
        this.log('{yellow-fg}Saving addresses.cache...{/yellow-fg}')
        fs.writeFile(PATH_ADDRESSES, this.addressOuts.getWhole(), (err) => {
          if (err) {
            reject(err)
          } else {
            resolve()
    ...
    this.appendAddressesCacheIfNeeded = () => {
      return new Promise((resolve, reject) => {
        this.log('{yellow-fg}Appending addresses.cache...{/yellow-fg}')
        if (this.addressOutsTail.getLength()) {
          fs.appendFile(PATH_ADDRESSES, this.addressOutsTail.getWhole(), (err) => {
            if (err) {
              reject(err)
            } else {
              resolve()
          ...
          this.log('{red-fg}addresses tail is empty{/red-fg}')
          resolve()
        }
      })
    }
  ...
  workWithCache(startCallback, endCallback) {
    ...
    storage.flush(() => {
      ...
      startCallback((rewrite = false) => {
        ...
        if (isChanged === 2) {
          this.writeSpendsCache()
          .then(() => this.writeTxMapCache())
          .then(() => this.writeBlockTimesCache())
          .then(() => this.writeCoinsCache())
          .then(() => this.writeAddressesCache())
          .then(() => {
          ...
          this.writeSpendsCache()
          .then(() => this.appendTxMapCacheIfNeeded())
          .then(() => this.appendBlockTimesCacheIfNeeded())
          .then(() => this.writeCoinsCache())
          .then(() => this.appendAddressesCacheIfNeeded())
          .then(() => {
            this.eachPlugin((instance, next) => {
              instance.onSaveCache(next)
            }, () => {
  ...
  add(block, callback, allowableLockCount = 0) {
    this.whenUnlocked((unlock) => {
      ...
      for (const i in blockUnpacked.txList) {
        const txData = blockUnpacked.txList[i].getData()
        ...
        txData.txOuts.each(({address}, outN) => {
          this.addAddressOut({
            address,
            blockId,
            txId: i,
  ...
  removeLast(count, callback, allowableLockCount = 0) {
    ...
    this.whenUnlocked((unlock) => {
      ...
      this.eachFromTo(this.length - count, this.length, ({id, hash, data}, next) => {
        ...
        this.addressOuts.filter(({blockId}) => blockId < this.length)
        this.eachPlugin((instance, next) => {
  ...
  addAddressOut(addressInfo, appendTail = true) {
    const tail = this.addressOuts.push(addressInfo)
    appendTail && this.addressOutsTail.addBuffer(tail)
  }
              ...
              this.loadCached(PATH_TIMES, 'blockTimes', (loaded) => {
                if (loaded) {
                  this.loadCached(PATH_COINS, 'coins', (loaded) => {
                    if (loaded) {
                      this.loadCached(PATH_ADDRESSES, 'addressOuts', (loaded) => {
                        if (loaded) {
                          if (this.length) {
  ...
  cache(callbacks) {
    ...
    this.each(({id, hash, data}, toReturn) => {
      ...
      for (const i in blockUnpacked.txList) {
        ...
        txUnpacked.txOuts.each(({address}, outN) => {
          this.addAddressOut({
            address,
            blockId: id,
            txId: i,
      ...
      .then(() => {
        Files.needDir(path, () => {
          this.whenUnlocked((unlock) => {
            ...
            .then(() => Files.copy(PATH_ADDRESSES, path + NAME_ADDRESSES))
  ...
  loadCheckpoint(callback) {
    ...
    if (fs.existsSync(path + 'ready')) {
      ...
      .then(() => Files.copyBack(PATH_SPENDS, path + NAME_SPENDS))
      .then(() => Files.copyBack(PATH_TXMAP, path + NAME_TXMAP))
      .then(() => Files.copyBack(PATH_TIMES, path + NAME_TIMES))
      .then(() => Files.copyBack(PATH_COINS, path + NAME_COINS))
      .then(() => Files.copyBack(PATH_ADDRESSES, path + NAME_ADDRESSES))
      .then(() => new Promise((resolve) => {
        this.eachPlugin((instance, next) => {
