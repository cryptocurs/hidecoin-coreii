...
class Blockchain extends Component {
  ...
  constructor() {
    ...
    this.addToLastBlocksHashesIds = (id, hash) => {
      ...
      this.lastBlocksHashesIds.filter((item) => item.added < timeLocal - 30)
      this.lastBlocksHashesIds.push({id, hash, added: timeLocal})
    }
  ...
  add(block, callback, allowableLockCount = 0) {
    this.whenUnlocked((unlock) => {
      ...
      const index = this.lastBlocksHashesIds.indexOf('hash', hash)
      if (index >= 0) {
        this.lastBlocksHashesIds.replace(index, {id: blockId, hash, added: Time.local()})
      }
      
      const indexRecord = Buffer.allocUnsafe(40)
      hash.copy(indexRecord)
  ...
  getBlockIdByHash(hash, callback, allowableLockCount = 0) {
    this.whenUnlocked((unlock) => {
      ...
      if (item) {
        unlock()
        callback(item.id === 0xffffffff ? -1 : item.id)
      } else {
        this.rEachHash(({id, hash: blockHash}, next) => {
          next(blockHash.equals(hash) ? id : undefined)
        }, (id) => {
          this.addToLastBlocksHashesIds(id >= 0 ? id : 0xffffffff, hash)
          unlock()
          callback(id)
        }, -1, allowableLockCount + 1)
      }
    }, allowableLockCount)
  }
  ...
  rEachHash(itemCallback, returnCallback, returnDefault = null, allowableLockCount = 0) {
    this.whenUnlocked((unlock) => {
      let id = this.length - 1
      this.indexCached.seek(id * 40)
      const ret = (res) => {
        unlock()
        returnCallback && returnCallback(res)
      }
      ...
      const readNext = () => {
        if (id >= 0) {
          const hash = this.indexCached.readBuffer(32)
          this.indexCached.forward(-72)
          itemCallback({id, hash}, (res) => {
            if (res !== undefined) {
              ret(res)
            } else {
              id--
  ...
  loadCacheIfExists(callback) {
    this.eachPlugin((instance, next) => {
      if (instance.onLoadCache) {
        instance.onLoadCache(next)
      } else {
        next()
      }
    }, () => {
      ...
      Files.removeDir(path)
        .then(() => {
          Files.needDir(path, () => {
            ...
            new Promise((resolve) => {
              this.eachPlugin((instance, next) => {
                instance.onBeforeSaveCheckpoint && instance.onBeforeSaveCheckpoint(next)
              }, () => {
                resolve()
              })
            })
              ...
              .then(() => new Promise((resolve) => {
                this.eachPlugin((instance, next) => {
                  instance.onSaveCheckpoint && instance.onSaveCheckpoint(path, next)
                }, () => {
                  resolve()
                })
              }))
  ...
  loadCheckpoint(callback) {
    ...
    if (fs.existsSync(path + 'ready')) {
      ...
      .then(() => new Promise((resolve) => {
        this.eachPlugin((instance, next) => {
          instance.onLoadCheckpoint && instance.onLoadCheckpoint(path, next)
        }, () => {
          resolve()
        })
      }))
