          ...
          blockchain.getLength((blockchainLength) => {
            WCache.create(this.login, (wcache) => {
              ...
              */
              ...
              blockchain.work((finished) => {
                blockchain.eachAddressOut(({address: addressRaw, blockId, txId, outN}, i, raw, next) => {
                  if (!(blockId % 100)) {
                    ifc.updateWindow('loading', {info: 'Caching wallet data (first use of wallet)...' + ((blockId * 1000 / blockchainLength >> 0) / 10) + '%'})
                  }
                  let found = false
                  for (const i in this.addresses) {
                    const address = this.addresses[i]
                    if (addressRaw.equals(address.getRaw())) {
                      blockchain.workingGetRaw(blockId, ({hash, data}) => {
                        const block = Block.fromRaw(hash, data)
                        const blockUnpacked = block.getData()
                        const tx = blockUnpacked.txList[txId]
                        const txHash = tx.getHash()
                        const txUnpacked = tx.getData()
                        const {value: amount} = txUnpacked.txOuts.get(outN)
