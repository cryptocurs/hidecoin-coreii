...
module.exports = class Wallet extends Component {
  ...
  getBalances(callback, allowableLockCount = 0) {
    blockchain.whenUnlocked((unlock) => {
      ...
      const balances = {}
      const addressHashes = R.map(address => address.getHash(), this.addresses)
      R.forEach((addressHash) => {
        balances[addressHash] = 0
      }, addressHashes)
      db.each("SELECT address, SUM(amount) amount FROM outs WHERE address IN (" + R.join(',', R.repeat('?', addressHashes.length)) + ") AND spent_at=? GROUP BY address", [...addressHashes, -1], ({address, amount}) => {
    ...
    blockchain.whenUnlocked((unlock) => {
      blockchain.getLength((blockchainLength) => {
        ...
        R.forEach((addressHash) => {
          ...
          db.each("SELECT address, SUM(amount) amount FROM outs WHERE block_height>? AND address IN (" + R.join(',', R.repeat('?', addressHashes.length)) + ") AND spent_at=? GROUP BY address", [blockchainLength - MIN_CONFIRMATIONS, ...addressHashes, -1], ({address, amount}) => {
          balances[address] = amount
        }, () => {
          unlock()
          callback && callback(balances)
