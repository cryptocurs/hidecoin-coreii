    ...
    Time.synchronize((timeOffset) => {
          ...
          p2p.online(storage.config.net && storage.config.net.server && storage.config.net.server.port || 7438, () => {
            ...
            ifc.key('C-y', () => {
              blockchain.getPrevBlockForNextBlock((hash) => {
                net.requestBlocksAfter(hash, 1, (err, res) => {
                  this.log({err, res})
                })
              })
            }, () => {
              this.log('requestBlocksAfter finished')
            })
            
            ifc.key('C-u', () => {
              net.clearRequestBlocksAfter()
              this.log('clearRequestBlocksAfter')
            })
      ...
      blockchain.loadCacheIfExists((loaded) => {
        if (loaded) {
          ...
          ifc.updateWindow('loading', {info: 'Restoring from checkpoint...'})
          blockchain.loadCheckpoint((loaded) => {
            if (loaded) {
              next()
            } else {
